...
public:
    ...
    explicit FakeStream(const std::string &initial_data)
    {
        load(initial_data);
    }
    ...
    const std::vector<char> &writtenData() const
    {
        return written_;
    }

    void failWrites(bool value = true)
    {
        fail_writes_ = value;
    }
...
TEST(HydrolibStreambufTest, OverflowReturnsEofWhenArgumentIsEof)
{
    ...
    EXPECT_EQ(streambuf.overflow(std::char_traits<char>::eof()), std::char_traits<char>::eof());
