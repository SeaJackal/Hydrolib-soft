...
//     requires(T logger, LogLevel level, strings::StaticFormatableString message,
//              Ts... params) { logger.WriteLog(level, message, params...); };

template <typename T, typename... Ts>
concept LogDistributorConcept =
    requires(T distributor, unsigned source_id, Log &log, Ts... params) {
...
{
public:
    consteval Logger(const char *name, unsigned id,
...
public:
    ...
    void WriteLog(LogLevel level, strings::StaticFormatableString message,
                  Ts... params)
    {
        Log log{.message = message, .level = level, .process_name = &name_};
