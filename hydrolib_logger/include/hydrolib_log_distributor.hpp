...
namespace hydrolib::Logger
{
    ...
    private:
        ...
        public:
            ...
            hydrolib_ReturnCode Push(const uint8_t *source, std::size_t length) const
            {
                hydrolib_ReturnCode self_res = HYDROLIB_RETURN_OK;
                hydrolib_ReturnCode other_res = next_node_->Push(source, length);
                if (stream_enable_)
                {
        ...
        public:
            template <typename... Ts>
            bool Notify([[maybe_unused]] unsigned source_id,
    ...
    consteval LogDistributor<Streams...>::LogDistributor(char *format_string, Streams &...streams)
        : distributing_list_(nullptr, streams...),
