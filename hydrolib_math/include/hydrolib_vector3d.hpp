...
inline Vector3D<Number> Vector3D<Number>::Cross(Vector3D<Number> &other) const {
  return Vector3D<Number>(y * other.z - z * other.y, z * other.x - x * other.z,
                          x * other.y - y * other.x);
}
...
template <ArithmeticConcept Number>
inline Vector3D<Number> Vector3D<Number>::operator+(
    Vector3D<Number> other) const {
  auto result = *this;
  result += other;
  return result;
}
