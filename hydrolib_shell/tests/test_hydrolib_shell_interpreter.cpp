...
class RecordingHandlersMap {
 public:
  ...
  std::optional<std::function<int(int, char *[])>> operator[](
...
TEST(InterpreterProcess, TrimsLeadingSpaces) {
  ...
  EXPECT_EQ((std::vector<std::string>{"execute", "paramX", "paramY", "paramZ"}),
...
TEST(InterpreterProcess, HandlesCommandWithoutArguments) {
  ...
  EXPECT_EQ((std::vector<std::string>{"start"}), handlers.invocations[1].args);
